...
libMallocInstrumented.so: mm.c list.c memlib.c mallocanalysis.c
	...
	LD_LIBRARY_PATH=$(INST_BUILD_LOC)/lib opt -load-pass-plugin libLLVMMallocInjectorPass.so -p mallocinjector mm.bc -o mm-instrumented.bc
	llc $(LLCFLAGS) mm-instrumented.bc -o mm-instrumented.o

	# This compiles and instruments list.c
	clang $(CLANGFLAGS) list.c -o list.bc
	LD_LIBRARY_PATH=$(INST_BUILD_LOC)/lib opt -load-pass-plugin libLLVMMallocInjectorPass.so -p mallocinjector list.bc -o list-instrumented.bc
